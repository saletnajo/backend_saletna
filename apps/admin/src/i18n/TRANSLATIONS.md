# Editing translations (admin panel)

This folder is the single place to change wording in the admin panel.

## How it works

The full UI translations — including the complete Arabic catalog — ship inside
the `@mercurjs/admin` package. At build time the dashboard plugin exposes this
...
1. Find the key in `reference/en.json` (search by the English text you see in
   ...
   readable copies of the catalogs bundled inside `@mercurjs/admin@2.1.6` —
   reference only, never loaded; regenerate them after a package upgrade.
   Upstream source:
   https://github.com/mercurjs/mercur/tree/main/packages/admin/src/i18n/translations
...
`@mercurjs/admin` and is not extensible from this app. If a new language must
